...
adata_sub.write("~/OSCC_visium_expression_matrix.h5ad", compression="gzip")

# Check
adata_check = sc.read_h5ad("~/OSCC_visium_expression_matrix.h5ad")
print(adata_check)
...
##### 2. Run SCRIPro
'''
scripro enrich_rna -i OSCC_visium_expression_matrix.h5ad -n 50 -s hg38 -p OSCC_visium -t 8
'''
...
edge_act = tf_activity_matrix.loc['edge']
core_act = tf_activity_matrix.loc['core']
edge_p = pval_matrix.loc['edge']
core_p = pval_matrix.loc['core']

common_tfs_up = core_act.index.intersection(core_p.index)
upstream_result = pd.DataFrame({'core_activity': core_act.loc[common_tfs_up], 'p-value': core_p.loc[common_tfs_up], 'label': 'upstream'})
common_tfs_down = edge_act.index.intersection(edge_p.index)
downstream_result = pd.DataFrame({'edge_activity': edge_act.loc[common_tfs_down], 'p-value': edge_p.loc[common_tfs_down], 'label': 'downstream'})
