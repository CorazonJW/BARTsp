# conda activate /home/jw4xtu/miniconda3/envs/SpaTrack
...
df = pd.read_csv("~/peak_expression_matrix.csv", index_col=0)
...
plt.savefig("~/ATAC_ptime_spatial_plot.png", dpi=600, bbox_inches='tight')
...
df_sig_res = df_res.loc[(df_res['model_fit']>0.01) & (df_res['fdr']<0.1) & (df_res['pvalue']<0.1)]
df_sig_res.to_csv("~/ATAC_spatrack_sort_exp_sig_with_trend.tsv", sep="\t")
...
sort_exp_sig.to_csv("~/ATAC_spatrack_sort_exp_sig.tsv", sep="\t")



##### 4. Predict trajectory-related transcription factors
ptime_df = adata_sub.obs[["ptime"]]
ptime_df.to_csv("~/ATAC_pseudotime.tsv", sep="\t")

adata_sub = adata_sub[:, ~adata_sub.var_names.duplicated()].copy()
adata_sub.write("~/ATAC_spatrack_dedup.adata.h5ad")

# only use peaks with decreased accessibility to predict upstream-active TFs
adata = sc.read_h5ad("~/ATAC_spatrack_dedup.adata.h5ad")
DEGs = pd.read_csv("~/ATAC_spatrack_sort_exp_sig_with_trend.tsv", sep = "\t")
...
adata_decreasing.write("~/ATAC_spatrack_decrease.adata.h5ad")

# only use peaks with increased accessibility to predict upstream-active TFs
...
adata_increasing.write("~/ATAC_spatrack_increase.adata.h5ad")

gr = spt.Trainer(data_type="p_time", expression_matrix_path="~/ATAC_spatrack_decrease.adata.h5ad", tfs_path="~/SpaTrack_ms.TF.tsv", ptime_path="~/ATAC_pseudotime.tsv", min_cells=0)
gr = spt.Trainer(data_type="p_time", expression_matrix_path="~/ATAC_spatrack_increase.adata.h5ad", tfs_path="~/SpaTrack_ms.TF.tsv", ptime_path="~/ATAC_pseudotime.tsv", min_cells=0)
# no peaks left for increase pattern
